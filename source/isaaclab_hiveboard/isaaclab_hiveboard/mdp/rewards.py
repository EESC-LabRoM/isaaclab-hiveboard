...
def ee_handle_dist(env: ManagerBasedRLEnv, threshold: float = 0.03) -> torch.Tensor:
    r"""Reward the robot for reaching the drawer handle using square law.

    When the ee is close enough, the reward becomes constant.
    """
    ee_tcp_pos = env.scene["ee_frame"].data.target_pos_w[..., 0, :]
    handle_pos = env.scene["cabinet_frame"].data.target_pos_w[..., 0, :]

    # Compute the squared Euclidean distance
    squared_distance = torch.sum(torch.square(handle_pos - ee_tcp_pos), dim=-1)

    # Create a threshold tensor with the same shape as the squared distance
    threshold_tensor = torch.tensor(threshold**2, device=squared_distance.device)

    # Reward is 0 if squared distance is less than the squared threshold
    reward = torch.where(squared_distance <= threshold_tensor,
...
def align_ee_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    .. math::
        ...
        reward = 0.5 * (align_y^2 + align_x^2)

    where :math:`align_y` is the dot product of the y direction of the gripper and the y direction of the handle
    ...
    ee_tcp_x, ee_tcp_y = ee_tcp_rot_mat[..., 0], ee_tcp_rot_mat[..., 1]
...
def align_grasp_around_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    handle_pos_z = env.scene["cabinet_frame"].data.target_pos_w[..., 0, 2]
    
    # Fingertips position: (num_envs, n_fingertips, 3)
    ee_fingertips_w = env.scene["ee_frame"].data.target_pos_w[..., :, :]
    top_finger_pos_z = ee_fingertips_w[..., 1, 2]
    bottom_finger_pos_z = ee_fingertips_w[..., 2, 2]

    # Check if hand is in a graspable pose
    is_graspable = (bottom_finger_pos_z < handle_pos_z) & (top_finger_pos_z > handle_pos_z)
...
def approach_gripper_handle(env: ManagerBasedRLEnv, offset: float = 0.04) -> torch.Tensor:
    ...
    top_finger_pos = ee_fingertips_w[..., 0, :]
    bottom_finger_pos = ee_fingertips_w[..., 1, :]

    # Compute the distance of each finger from the handle
    top_finger_dist = torch.abs(top_finger_pos[:, 2] - handle_pos[:, 2])
    bottom_finger_dist = torch.abs(bottom_finger_pos[:, 2] - handle_pos[:, 2])

    # Check if hand is in a graspable pose
    is_graspable = (bottom_finger_pos[:, 2] < handle_pos[:, 2]) & (top_finger_pos[:, 2] > handle_pos[:, 2])

    return is_graspable * ((offset - top_finger_dist) + (offset - bottom_finger_dist))


def close_handle_when_near(
...
def desired_contacts(env: ManagerBasedRLEnv, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    """Reward desired contacts above a threshold."""
