...
"""Isaac Lab 3 configuration for the kitless ANYmal + DynaArm small-valve task."""
...
from isaaclab_hiveboard.tasks.anymal.small_valve.configs.commands import FramePoseCommandsCfg
from isaaclab_hiveboard.tasks.anymal.small_valve.configs.events import ValveEventCfg, ValvePlayEventCfg
...
@configclass
class AnymalSmallValvePhysicsCfg(PresetCfg):
...
@configclass
class AnymalSmallValveEnvCfg(ManagerBasedRLEnvCfg):
    """Randomized fixed-base ANYmal + DynaArm task that spins the small gate-valve handwheel."""
    ...
    sim: SimulationCfg = SimulationCfg(dt=1 / 200, render_interval=1, physics=AnymalSmallValvePhysicsCfg())  # type: ignore
    rewards = None
...
@configclass
class AnymalSmallValveEnvCfg_PLAY(AnymalSmallValveEnvCfg):
