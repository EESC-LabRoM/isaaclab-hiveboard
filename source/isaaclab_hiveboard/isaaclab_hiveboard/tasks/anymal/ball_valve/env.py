...
"""Isaac Lab 3 configuration for the kitless ANYmal + DynaArm ball-valve task."""
...
from isaaclab_hiveboard.tasks.anymal.ball_valve.configs.commands import FramePoseCommandsCfg
from isaaclab_hiveboard.tasks.anymal.ball_valve.configs.events import ValveEventCfg, ValvePlayEventCfg
...
@configclass
class AnymalBallValvePhysicsCfg(PresetCfg):
...
class AnymalBallValveEnvCfg(ManagerBasedRLEnvCfg):
    """Randomized fixed-base ANYmal + DynaArm task that physically opens the valve to -90 degrees."""
    ...
    sim: SimulationCfg = SimulationCfg(dt=1 / 200, render_interval=1, physics=AnymalBallValvePhysicsCfg())  # type: ignore
    rewards = None
...
@configclass
class AnymalBallValveEnvCfg_PLAY(AnymalBallValveEnvCfg):
