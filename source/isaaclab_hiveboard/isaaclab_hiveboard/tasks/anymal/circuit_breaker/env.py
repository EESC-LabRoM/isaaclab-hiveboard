...
"""Isaac Lab 3 configuration for the kitless ANYmal + DynaArm circuit-breaker task."""
...
from isaaclab_hiveboard.tasks.anymal.circuit_breaker.configs.commands import FramePoseCommandsCfg
from isaaclab_hiveboard.tasks.anymal.circuit_breaker.configs.events import (
    CircuitBreakerEventCfg,
    CircuitBreakerPlayEventCfg,
)
from isaaclab_hiveboard.tasks.anymal.circuit_breaker.configs.observations import ObservationsCfg
from isaaclab_hiveboard.tasks.anymal.circuit_breaker.configs.scene import CircuitBreakerSceneCfg
from isaaclab_hiveboard.tasks.anymal.circuit_breaker.configs.terminations import TerminationsCfg


@configclass
class AnymalCircuitBreakerPhysicsCfg(PresetCfg):
...
@configclass
class AnymalCircuitBreakerEnvCfg(ManagerBasedRLEnvCfg):
    """Randomized fixed-base ANYmal + DynaArm task that flips the circuit-breaker switch."""
    ...
    sim: SimulationCfg = SimulationCfg(dt=1 / 200, render_interval=1, physics=AnymalCircuitBreakerPhysicsCfg())  # type: ignore
    rewards = None
...
@configclass
class AnymalCircuitBreakerEnvCfg_PLAY(AnymalCircuitBreakerEnvCfg):
